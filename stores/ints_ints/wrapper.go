...
*   $ fs2-generic \
*     --output=wrapper.go \
*     --package-name=ints_ints \
*     bptree \
*     --key-type=[]int32 \
*     --key-serializer=SerializeInt32s \
*     --key-deserializer=DeserializeInt32s \
*     --value-type=[]int32 \
*     --value-serializer=SerializeInt32s \
*     --value-deserializer=DeserializeInt32s
*
...
func (b *BpTree) Add(key []int32, val []int32) error {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	return b.bpt.Add(SerializeInt32s(key), SerializeInt32s(val))
}

func (b *BpTree) Count(key []int32) (int, error) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	return b.bpt.Count(SerializeInt32s(key))
}

func (b *BpTree) Has(key []int32) (bool, error) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	return b.bpt.Has(SerializeInt32s(key))
}
...
func (b *BpTree) kvIter(kvi fs2.Iterator) (it Iterator) {
	it = func() (key []int32, value []int32, err error, _ Iterator) {
		...
		key = DeserializeInt32s(k)
		value = DeserializeInt32s(v)
...
func (b *BpTree) Find(key []int32) (it Iterator, err error) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	raw, err := b.bpt.Find(SerializeInt32s(key))
	if err != nil {
		return nil, err
	}
...
func (b *BpTree) Range(from, to []int32) (it Iterator, err error) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	raw, err := b.bpt.Range(SerializeInt32s(from), SerializeInt32s(to))
	if err != nil {
		return nil, err
	}
	return b.kvIter(raw), nil
...
func (b *BpTree) Remove(key []int32, where func([]int32) bool) error {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	return b.bpt.Remove(SerializeInt32s(key), func(bytes []byte) bool {
