...
*     --value-deserializer=DeserializeEdge
*
...
type MultiMap interface {
	...
	DoFind(key int32, do func(int32, subgraph.Edge) error) error
	Range(from, to int32) (Iterator, error)
	DoRange(from, to int32, do func(int32, subgraph.Edge) error) error
	Has(key int32) (bool, error)
	Count(key int32) (int, error)
	Add(key int32, value subgraph.Edge) error
	Remove(key int32, where func(subgraph.Edge) bool) error
	Size() int
	Close() error
	Delete() error
}

type Iterator func() (int32, subgraph.Edge, error, Iterator)
type KeyIterator func() (int32, error, KeyIterator)
type ValueIterator func() (subgraph.Edge, error, ValueIterator)

func Do(run func() (Iterator, error), do func(key int32, value subgraph.Edge) error) error {
	kvi, err := run()
	if err != nil {
		return err
	}
	var key int32
	var value subgraph.Edge
...
func DoValue(run func() (ValueIterator, error), do func(subgraph.Edge) error) error {
	it, err := run()
	if err != nil {
		return err
	}
	var item subgraph.Edge
	for item, err, it = it(); it != nil; item, err, it = it() {
		e := do(item)
		if e != nil {
			return e
...
func (b *BpTree) valueIter(raw fs2.ItemIterator) (it ValueIterator) {
	it = func() (value subgraph.Edge, err error, _ ValueIterator) {
		b.mutex.Lock()
		defer b.mutex.Unlock()
		var i []byte
		i, err, raw = raw()
		if err != nil {
			return subgraph.Edge{}, err, nil
		}
		if raw == nil {
			return subgraph.Edge{}, nil, nil
		}
		value = DeserializeEdge(i)
		return value, nil, it
	}
	return it
}
...
func (b *BpTree) DoFind(key int32, do func(int32, subgraph.Edge) error) error {
...
func (b *BpTree) DoRange(from, to int32, do func(int32, subgraph.Edge) error) error {
